...
int main()
{
	...
	table->addChild(memory->createCushionEntity(CushionEntityParams{vec3(0, SNOOKER_TABLE_HEIGHT / 2, 0), vec3(0), vec3(0), cushionHorizontalMesh, defaultShader, "topCushion", vec3(0,-1,0), vec3(0,SNOOKER_TABLE_HEIGHT / 2, 0) }));
	//table->addChild(memory->createCushionEntity(CushionEntityParams{vec3(0, -(SNOOKER_TABLE_HEIGHT / 2), 0), vec3(0), vec3(0), cushionHorizontalMesh, defaultShader, "bottomCushion", vec3(0,1,0), vec3(0,-SNOOKER_TABLE_HEIGHT / 2, 0) }));
	//table->addChild(memory->createCushionEntity(CushionEntityParams{vec3(-(SNOOKER_TABLE_WIDTH / 2), 0, 0), vec3(0), vec3(0), cushionVerticalMesh, defaultShader, "leftCushion", vec3(1,0,0), vec3(-SNOOKER_TABLE_WIDTH / 2, 0, 0) }));
	//table->addChild(memory->createCushionEntity(CushionEntityParams{vec3((SNOOKER_TABLE_WIDTH / 2), 0, 0), vec3(0), vec3(0), cushionVerticalMesh, defaultShader, "rightCushion", vec3(-1,0,0), vec3(SNOOKER_TABLE_WIDTH / 2, 0, 0) }));
